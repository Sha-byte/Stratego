...
class info:
    ...
    def getmoves(self):
        ...
        if pos1:
            if pos1[0]+1 <= 2 and ([pos1[0]+1, pos1[1]] != pos2) and ([pos1[0]+1, pos1[1]] != flag) and not self.p1:
                moves['d1']=True
            if 0 <= pos1[0]-1 and ([pos1[0]-1, pos1[1]] != pos2) and ([pos1[0]-1, pos1[1]] != flag) and self.p1:
                moves['u1']=True
            if pos1[1] + 1 <= 2 and ([pos1[0],pos1[1]+1] != pos2) and ([pos1[0],pos1[1]+1] != flag):
                moves['r1']=True
            if 0 <= pos1[1]-1 and ([pos1[0],pos1[1]-1] != pos2) and ([pos1[0],pos1[1]-1] != flag):
                moves['l1']=True
        if pos2:
            if pos2[0]+1 <= 2 and ([pos2[0]+1, pos2[1]] != pos1) and ([pos2[0]+1, pos2[1]] != flag) and not self.p1:
                moves['d2']=True
            if 0 <= pos2[0]-1 and ([pos2[0]-1, pos2[1]] != pos1) and ([pos2[0]-1, pos2[1]] != flag) and self.p1:
                moves['u2']=True
            if pos2[1] + 1 <= 2 and ([pos2[0],pos2[1]+1] != pos1) and ([pos2[0],pos2[1]+1] != flag):
                moves['r2']=True
            if 0 <= pos2[1]-1 and ([pos2[0],pos2[1]-1] != pos1) and ([pos2[0],pos2[1]-1] != flag):
                moves['l2']=True
        n = len(moves)
        for i in moves:
            moves[i] = 1/n
...
class Strat:
    ...
    def train(self, n_iterations=200):
        ...
        p1list = [['1F', '11', '12'], ['1F', '12', '11'], ['11', '1F', '12'], ['12', '1F', '11'], ['11', '12', '1F'], ['12', '11', '1F']]
        p2list = [['2F', '21', '22'], ['2F', '22', '21'], ['21', '2F', '22'], ['22', '2F', '21'], ['21', '22', '2F'], ['22', '21', '2F']]
        for _ in range(n_iterations):
            print(_)
            for p1l in p1list:
                for p2l in p2list:
                    board = [[p2l[0], p2l[1], p2l[2]], ['', '', ''], [p1l[0], p1l[1], p1l[2]]]
                    info_set = (1, 'x', 'x', 'x', '', '', '', p1l[0], p1l[1], p1l[2])
    ...
    def get_info(self, key):
        if key not in self.nodeMap:
            getM = [[key[1], key[2], key[3]],[key[4], key[5], key[6]], [key[7], key[8], key[9]]]
